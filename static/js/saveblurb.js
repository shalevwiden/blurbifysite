...
saveblurb.addEventListener("click", () => {
  ...
  const blurb_and_bg = document.querySelector(".blurbify_bg_div");

  oldStyle = blurbdiv.getAttribute("style") || "";

  if (window.innerWidth <= 450) {
    // Do something if the screen width is 450px or less
    blurbdiv.style.cssText = "margin: 10px; transform: scale(.9);";
  } else {
    // Do something else for larger screens
    blurbdiv.style.cssText = "margin: 10px; ";
  }
  ...
  htmlToImage
    ...
    .then((dataUrl) => {
      ...
      blurbdiv.setAttribute("style", oldStyle);
